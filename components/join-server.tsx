...
export const JoinServer = () => {
  ...
  const actionCopyboard = () => {
    ...
    toast.success('IP успешно скопирован!', {
      ...
      transition: Zoom,
      icon: ({ theme, type }) => <img src="/images/minecraft/icons/book_big.webp" />,
    })
  }
    ...
    <Dialog>
      <DialogTrigger className="w-full">
        <div className="flex items-center justify-center relative w-full h-[54px] lg:h-[64px] rounded-md
          ...
          <p className=" text-white text-shadow-xl font-bold text-xl lg:text-2xl">Начать играть</p>
        </div>
      </DialogTrigger>
      <DialogContent className="max-w-7xl h-3/4 bg-transparent border-none">
        <div className="flex justify-center items-center p-12 bg-repeat border-4 border-black h-full w-full" style={{ backgroundImage: `url("/images/static/dirt.png")` }}>
          <div className="flex flex-col gap-y-6 justify-between">
            <div className="flex flex-col gap-y-2">
              ...
              <HoverCard closeDelay={0} openDelay={0}>
                ...
                <HoverCardContent className="bg-black p-2 border-none">
                  <span className="text-md text-neutral-400">Скопировать IP</span>
                </HoverCardContent>
              </HoverCard>
            </div>
            <div className="flex flex-col gap-y-2">
              <HoverCard closeDelay={0} openDelay={0}>
                <HoverCardTrigger>
                  <div className="button w-full text-center md:w-96 px-2 py-1 text-[0.8rem] lg:text-base">Наборы ресурсов: Включены</div>
                </HoverCardTrigger>
                <HoverCardContent className="w-[400px] bg-black p-2 border-none">
                  <span className="text-md text-neutral-400">На сервере используется свой ресурспак. Эту опцию рекомендуется оставить включенной!</span>
                </HoverCardContent>
              </HoverCard>
              <DialogClose>
                <div className="button w-full text-center md:w-96 px-2 py-1 text-[0.8rem] lg:text-base">Готово</div>
              </DialogClose>
            </div>
          </div>
        </div>
