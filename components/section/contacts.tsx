		...
		<div className="full-screen-section flex flex-col items-center justify-center bg-background-dark py-24 xl:py-0">
			<Wrapper>
				<div className="flex flex-col gap-y-12">
					<h1 className='text-3xl md:text-6xl lg:text-6xl xl:text-7xl 2xl:text-8xl text-rose-500 text-center'>
							...
							<div key={item.name} className="flex p-1 flex-col justify-between block-item">
								<div className="bg-wool-black p-5">
									...
									{item.content.map((subItem, idx) => (
										<div key={idx}>
											{subItem.minuses && subItem.minuses.map((minus, minusIndex) => (
												<p className="md:text-xl" key={minusIndex}>
													&gt; {minus}
												</p>
											))}
										</div>
									...
									<div
										...
										className="flex flex-row items-center gap-x-4 brightness-110 mt-4 py-4 cursor-pointer group"
									>
										<item.icon
											size={32}
											className={`${item.name === "Телеграмм"
												? 'group-hover:fill-[#229ED9]'
												: 'group-hover:fill-discord-color'} group-hover:transition fill-white`}
										/>
										<span className={`${item.name === "Телеграмм"
											? 'group-hover:text-[#229ED9]'
											: 'group-hover:text-discord-color'} text-base text-white md:text-xl group-hover:transition`}
										>
											Заходи в {item.name}!
										</span>
									</div>
