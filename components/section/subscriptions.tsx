...
export const Subscriptions = () => {
  ...
  const SubList = support.Support.content[0].donateElement.map((item) => ({
    title: item.title, level: item.level, desc: item.description, commands: item.commands, functions: item.functions, price: item.price, linkToBuy: item.link
  }));
  ...
  const tabsList = tabsListData.map((item) => (
    <TabsTrigger
      key={item.name}
      value={item.value}
      className="raised-slot-button cursor-pointer p-0 md:p-2 lg:p-3 w-full py-2 md:text-3xl text-xl text-zinc-700 hover:text-zinc-900"
    >
      {item.name}
    </TabsTrigger>
  ))
  ...
  return (
      <div className="flex flex-col py-24" >
        ...
        <Tabs defaultValue="subscriptions" className="flex flex-col gap-y-12">
          <TabsList className="flex flex-col lg:flex-row gap-4 items-start justify-center bg-transparent mb-12">
            {tabsList}
          </TabsList>
